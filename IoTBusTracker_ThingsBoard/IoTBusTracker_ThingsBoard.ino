...
void getLocation() 
{
  ...
  //WebSerial.println(location.getSurroundingWiFiJson()+"\n");
  WebSerial.println ("Latitude: " + String(loc.lat, 7));
  WebSerial.println("Longitude: " + String(loc.lon, 7));
  //WebSerial.println ("Accuracy: " + String (loc.accuracy));
  //WebSerial.println ("Result: " + location.wlStatusStr (location.getStatus ()));

  if (millis() - previousDataSend > telemetrySendInterval) 
  {
    previousDataSend = millis();
    tb.sendTelemetryData("latitude", String(loc.lat, 7));
    tb.sendTelemetryData("longitude", String(loc.lon, 7));
